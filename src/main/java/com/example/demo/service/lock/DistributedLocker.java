...
public class DistributedLocker {
	...
	public <T> LockExecutionResult<T> lock(final String key, final int howLongShouldLockBeAcquiredSeconds,
		...
		try {
			return tryToGetLock(() -> {
				final Boolean lockAcquired = valueOperations.setIfAbsent(key, key, lockTimeoutSeconds, TimeUnit.SECONDS);
	...
	private static <T> T tryToGetLock(final Supplier<T> task, final String lockKey,
